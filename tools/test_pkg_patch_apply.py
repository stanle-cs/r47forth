...
class TestSameFunctionConflictTwoPackages(unittest.TestCase):
    """Unit 6 / §7 hard invariant, end to end through the REAL
    pipeline: both packages' patches are generated by the real refresh
    (authoring path), collected/ordered by collect_patch_stacks and
    applied by apply_patch_stack (build path) — not `git apply` in
    isolation. All fixtures are constructed in temp dirs by the test
    itself and torn down with it; no synthetic packages are committed.
    """

    def _two_package_repo(self, edit_a, edit_b):
        """Repo with pkg-a and pkg-b patches generated via refresh()
        from the given materialized edits."""
        repo = _TempGitRepo()
        repo.__enter__()
        repo.write_src(UPSTREAM_3FN)
        specs = []
        for pkg, text in (('packages/pkg-a', edit_a),
                          ('packages/pkg-b', edit_b)):
            os.makedirs(os.path.join(repo.tmpdir, pkg), exist_ok=True)
            with open(os.path.join(repo.tmpdir, pkg, 'test.c'), 'w') as f:
                f.write(text)
            from pkg_patch_refresh import refresh
            written = refresh(pkg, 'test.c', repo.tmpdir, repo.cc_path)
            specs.append((pkg, written))
        return repo, specs

    def test_same_function_conflict_fails_loudly_end_to_end(self):
        """BUG THIS TEST EXISTS TO CATCH: two packages editing the SAME
        function with divergent content getting silently resolved by
        picking one package's version (today's last-listed-wins
        behavior). The full pipeline must raise, naming the losing
        patch, and produce no output file.

        Mutation: skip the conflict-marker scan AND ignore git apply's
        exit status in apply_patch_stack — the build would ship pkg-a's
        version (or marker-corrupted source) with no signal; this test
        fails."""
        edit_a = _edit_function(UPSTREAM_3FN,
                                'return x + 2;', 'return x * 100;')
        edit_b = _edit_function(UPSTREAM_3FN,
                                'return x + 2;', 'return x - 5;')
        repo, specs = self._two_package_repo(edit_a, edit_b)
        try:
            stacks = collect_patch_stacks(specs, repo.tmpdir)
            self.assertEqual(len(stacks['test.c']), 2,
                             'both packages contribute to the stack')
            ...
            self.assertIn('pkg-b', msg,
                          'error must name the conflicting patch')
            self.assertFalse(os.path.exists(dest),
                             'no output may reach the compiler')
        finally:
            repo.__exit__()

    def test_different_function_edits_compose_cleanly(self):
        """Contrast case guarding against over-broad failure: the same
        two packages editing DIFFERENT functions must compose
        cumulatively (this is the whole point of §3 — mere same-file
