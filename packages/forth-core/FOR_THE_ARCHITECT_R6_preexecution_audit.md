...
|---|---|---|---|---|---|---|
| `packages/forth-core/DESIGN.md` | authoritative design | 1 | rev of 75506c49d (2026-07-15 19:55) | F | **current, with stale/conflicting passages** (§5) | everything |
...
| `custom_package/README.md` | package-system authority | 3 | post-R5 | F | current; one stale self-contradiction (AUD-L2) | authoring |
| `custom_package/PROPOSED_SPEC_CHANGES.md` | pkg-mgr spec rev 2 + BP/R5 approvals | 4 | post-R5-2 | S | current for pkg-mgr; implemented | tooling |
| `packages/forth-core/PROPOSED_SPEC_CHANGES.md` | fc spec proposals | 4 | 2026-07-13 | F | item 1 ratified (upstream MR pending); item 2 **stale-"PROPOSED"** — already ratified into DESIGN §3.3.6 (AUD-L3) | — |
...
| `QWEN_PROMPTS_R5_tooling.md` | pkg tooling hardening (R5-1..7) | 5 | 2026-07-15 | F | executed (e579d1821 = R5-6 message; R5-7 code + tests verified in `tools/`) | — |
...
| `custom_package/QWEN_IMPLEMENTATION_PROMPTS.md` | **revision-1 (libclang)** pkg prompts | 5 | pre-revert | S | **superseded, unmarked** — execution hazard (AUD-M5) | none (must stay unused) |
| `custom_package/QWEN_PROMPTS_refresh-base-commit.md` | BP-1..BP-7 base pinning | 5 | 2026-07-14 | S | executed (verified in `tools/pkg_patch_refresh.py` + tests) | — |
...
| `FOR_THE_ARCHITECT_R5.md` | tooling policy A1–A3 | 5 | 2026-07-15 | F | A1 approved+implemented (R5-7); A2 implemented (e688535e2); A3 ruled accepted-silent (README) | — |
| `manager-audit.md` | audit of the *old* declaration/resolver system | 7 | pre-redesign | F | superseded (system replaced twice); surviving ideas landed (sentinel, stubs, test_asan docs) | — |
...
| `custom_package/IMPLEMENTATION_REPORT.md` | pkg-mgr implementation record | 5 | Jul 15 | S | current record; its "not independently audited" banner is now satisfied by R5 | — |
...
- **AUD-M5 — `custom_package/QWEN_IMPLEMENTATION_PROMPTS.md` is the reverted
  revision-1 (libclang) prompt set with no supersession banner** (Prompt 2 =
  "Function-Boundary Extractor (libclang)", :215) while
  `custom_package/PROPOSED_SPEC_CHANGES.md:1-8` declares revision 2 supersedes
...
- **AUD-L2** `custom_package/README.md` Limitations (:506-509) still claims "No
  existing package uses this system yet — forth-core needs migration",
  contradicting Current Status (:16-28).
- **AUD-L3** `packages/forth-core/PROPOSED_SPEC_CHANGES.md:20-22` marks the
...
**10.4 custom_package/QWEN_IMPLEMENTATION_PROMPTS.md:** add the ARCHIVED
