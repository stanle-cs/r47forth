...
Identical to F3-1's preamble with paths renamed to `f3-5`
(`/tmp/forth-f3-5-todo.md`, `-gate.log`, `-mutN.log`).  All nine rules and
the two-attempt handoff apply verbatim.  (No program fixtures are authored
here — the fixture-rule block is not required.)
