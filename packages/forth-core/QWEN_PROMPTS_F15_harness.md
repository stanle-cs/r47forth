...
|---|---|---|---|---|
| F15-1 end-to-end run lifecycle | `QWEN_PROMPTS_F15_1_run_lifecycle.md` | **READY TO EXECUTE** | 1, 7(a,b), 9(a,b) | F1 complete + `6345f6c64` |
| F15-2 entry state + power-off round-trip | — | TO AUTHOR (verification pass pending) | 2(a-d) | F15-1 committed green |
...
Packets F15-2..F15-5 are deliberately **not** authored yet: each needs its
own architect verification pass against the tree its predecessor leaves
(rule 1 above). Design notes fixed so far, from the completed inventory:

- **F15-2:** `forthEntryStateAtCursor` / `forthEntryStateAtInsertion`
  (forth_bridge.c) are pure probes over `currentStep`/program bytes; model
  is `test_entry_state_derivation` (unit analog) extended with real cursor
  motion (`fnGotoDot`) and the 2(d) power-off round-trip modeled on
  `test_save_restore_roundtrip`. Mutation: the §8.9 item-2 static-bool
  replacement of `forthEntryStateAtInsertion`.
...
- **F15-4:** glyphs are two-byte sequences — `STD_CROSS` `"\x80\xd7"`,
  ...
  strcmp; the packet must use rename-to-sentinel instead. Item 6's RPN half
  drives `addItemToNimBuffer` (src/c47/bufferize.c:837) — the exact digit
  item and NIM-close call must be verified in the F15-4 pass.
