...
#define FORTH_CONSOLE_ROW_PITCH   21   /* the fnPem listing pitch, not
                                          STANDARD_FONT_HEIGHT (22, the font
                                          browser's metric) */

/* The band's row counts are the geometry this module was designed and
 * tested against (4 short, 2 long).  Tie them to UPSTREAM's names, so
 * that moving a register line or the NIM line upstream breaks this
 * build instead of silently repainting the transcript over the editor.
 * checkHPoffset is a runtime term and cannot appear here; the default
 * layout (checkHPoffset == 0) is what is asserted, which is the layout
 * every landed test drives. */
...
/* rows is a VIEW concept the ring module deliberately does not have, so
 * the view owns both the row count and the roll's upper bound. */
...
/* The largest view offset that still fills the band; the roll is no
 * longer its only enforcer — see _forthConsoleClampView. */
static uint16_t _forthConsoleMaxView(uint16_t rows, uint16_t count) {
  return (count > rows) ? (uint16_t)(count - rows) : 0;
}

/* Clamping only where the offset is WRITTEN is correct only while the
 * bound is constant — and it is not: rows is frame-variable, because the
 * editor's long-line state halves the band from 4 rows to 2.  An offset
 * clamped legally at count-2 with a long line becomes illegal the moment
 * BACKSPACE shortens the line, no roll key pressed and nothing to
 * re-clamp it; the renderer's only guard is `view >= count -> continue`,
 * so the top rows painted nothing.
 *
 * Upstream has this exact class — a display window index that can
 * outrun its content — and solves it in two parts, neither of which is
 * a write during the paint: the WRITE clamp lives at the scroll site
 * (scrollPemBackwards/scrollPemForwards guard before they move), and the
 * paint TOLERATES an out-of-range index instead of rewriting it
 * (defineFirstDisplayedStep walks to the window and simply breaks if the
 * content runs out).
 *
 * So the write clamp below belongs to forthConsoleRollView — the roll IS
 * this band's scroll site — and the renderer clamps a LOCAL copy for the
 * frame (_forthConsoleViewBase).  Do not collapse these back into a
 * single write-time clamp: the band must stay full on every frame, and a
 * paint must never write state — a stored offset means "where the owner
 * left the view", not "where the last repaint happened to leave it". */
 ...
 * frame's own bound.  Pure — see the note above.
 *
 * Exported to the self-test build only (the keyboard.c executeFunction
 * precedent) so the geometry and roll cases can drive the paint offset
 * directly, without the menus, status bar and screenUpdatingMode
 * arithmetic a full refreshScreen() drags in.  The ARM being wired is
 * proven separately, through refreshScreen(), by test_console_view_arm.
 * The gate and the paint below need no such macro: screen.c's refresh
 * arm calls them across the file boundary, so they are plain
 * non-static. */
...
void forthConsoleRollView(int16_t delta) {
  ...
  /* the ring's roll stops at count-1 (its own bound: there is no older
   * line).  The VIEW stops at count-rows, so the band stays full — past
   * that, each press emptied the transcript one row at a time. */
 ...
 *    protect this arm: that bracket is around code that never refreshes.
 *  - lastErrorCode: the error text paints INSIDE _refreshRegisterLine on
 *    errorMessageRegisterLine (REGISTER_Z by default); an unconditional
 *    console would swallow every error display.
 ...
 *    — and they hang off the REGISTER_X paint the console KEEPS, so a live
 *    TI would wipe the transcript from inside the one call this arm does
 *    not own.  Reachable: a console line can execute any CAT_FNCT/PTP_NONE
 *    item, and BATTV/BYTES/WHO/VERS all set one. */
bool_t _forthConsoleActive(void) {
  /* LIVE, not origin — the render gate fired on the suspended residue
   * and painted TAM's abandoned aimBuffer as an editable console line. */
...
void _forthConsoleRender(void) {
   ...
   * (yincr-1) + 1*(yincr-1) = 68 with yincr 35, cursor from 67.
   *
  ...
  viewBase = _forthConsoleViewBase(rows, count);  /* rows is
   ...
   * short-line state, 2 at Y 25/46 in the long-line state. */
  ...
  for(r = 0; r < rows; r++) {
    ...
    /* Truncate with the native ellipsis; 14 px is STD_ELLIPSIS's width.
     * No wrapping — render-only, additive later. */
