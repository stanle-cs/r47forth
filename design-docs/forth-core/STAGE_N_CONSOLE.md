...
| N-R10 | Which key runs the line once the capture has a console face? (owner change 2026-08-10, landed in `d5f4811aa`/`8a3c6e146` and announced in the v0.3 post: "ENTER ran the line before the console landed, doesn't work well in new design") | **R/S RUNS, ENTER TYPES ONE SPACE.** ENTER's console meaning is the token separator — `processAimInput(ITM_SPACE)` in `fnKeyEnter`'s CM_AIM divert, metered by the same capture cap as any typed character, never a commit to X. R/S owns execution through `forthInteractiveRun()` (the renamed `forthInteractiveEnter`), reached from the `ITM_RS` guard at the head of the CM_AIM arm — before the BST/SST arm, which `ITM_RS` does not match. Both diverts are **LIVE-gated**: a suspended capture's `aimBuffer` is TAM's and keeps native ENTER, and every non-console CM_AIM line keeps the native commit. Everything else about the REPL act is unchanged and still N-R4's: FHIST push before the run, echo and push one act, reopen empty with keys-first relocked, error reopens with the line intact, empty is a no-op and not a close. Two supports the swap needs and now has: the open writes `FORTH_CONSOLE_CONTROL_HINT` (`"ENTER=SPACE  R/S=RUN"`) as an ordinary ring record, and the R/S arm takes a full `refreshScreen(142)` after clearing `SCRUPD_MANUAL_STACK`/`SCRUPD_SKIP_STACK_ONE_TIME`, because the landed CM_AIM repaint touches the input row only and the transcript would otherwise update one keypress late. Supersedes the N-R4 header wording ("what ENTER writes") and the L-R3 alias sentence ("R/S runs the line exactly as ENTER"); §8.4.2/§8.4.4 carry the normative text. |
