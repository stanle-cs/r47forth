...
| M-R5 | The catalog entry point | **A `-MNU_FORTH` row in the CATALOG menu table** (package softmenus.c, one table edit). The FIX-9/L1-1 catalog-drain predicates re-audited against the new reachability: dispositions all KEEP — the drain is `catalog`-variable-gated (forth_compile.c:1717) and menu-tree rows never set it, so a stacked FWRD/CATALOG legitimately survives beneath a capture and EXIT restores it (native closeAim shape; pinned by the M1-1 battery, which corrected the trace's own first claim here — see STAGE_M_TRACES.md M-T5 correction). |
