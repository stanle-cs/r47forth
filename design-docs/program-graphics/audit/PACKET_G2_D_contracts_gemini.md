...
Your one question: **do the commands keep their contract with the calculator around them?** Walk these: a coordinate register of type short integer, string, matrix, or complex where a real is expected; `ARC` with a real center in T instead of a complex; `ARC` with a long integer angle in Y and a real in X; `TEXTOUT` with an empty string, a string longer than the screen, a string of two-byte glyphs, and a point on the right edge of the clip; `DISP` with the view closed (the clip is the whole screen) and with region 2 for lines 8 to 11; `GCLIP` with the view closed, with corners reversed, and with a rectangle wholly outside the region; `GMODE` values from a program; the shared scratch buffer `tmpString` being overwritten by a caller that still needs it; the refresh cadence when `getUptimeMs` wraps; the `lastErrorCode` left by a refused command and what the next command does; the state left in `canvas` after each error return; the stack after each command (the contract says unchanged); and `pgReadCoord` when `getRegisterMaxDataLengthInBlocks` returns 0.
