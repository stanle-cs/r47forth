...
Your one question: **is the window arithmetic correct at its boundaries?** Walk the numbers: `pgRealToPixel` without a window for 2.5, -2.5, 0.5, -0.5, 32767.4, 32767.5, -32767.5, 1e30, -0 (negative zero), NaN; with XRNG 0 10 for x = 5, 10, 10.0000000001, -1e-40; with a range of width 1e-30 (xmax - xmin tiny) and with a width of 1e30; with a reversed range XRNG 10 0 for x = 0, 5, 10, 11; with XRNG -32767 32767 and x = 32767; the rounding step when the ratio is exactly n + 0.5 for negative and positive n, against `screenWindowRatio` (they must agree wherever upstream does not clamp); `realToInt32C47` when `t` after the 0.5 step is exactly 32767.5 or 2147483647.5; `pgReadReal` for a long integer with 40 digits (a real34 holds 34) and for a real34 of -0; `pgRange` when the two ends differ only beyond 34 digits, when one end is a long integer and the other a real, and when Y is not a number type; the axis bits of `pgWindow.set` when only YRNG was set and a real x arrives; `pgReadComplexPoint` for a complex whose real part is a pixel value and whose imaginary part is NaN (which error, and is anything half-drawn); `pgReadTwoPoints` for X complex and Y complex with Z and T holding strings (must Z and T be read at all); `fnGclip` when the two points arrive through the window (a clip set in user units). For every arithmetic claim give the numbers.
